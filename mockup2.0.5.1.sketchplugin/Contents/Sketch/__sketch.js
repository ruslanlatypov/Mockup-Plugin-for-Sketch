...
module.exports = "file://" + String(context.scriptPath).split(".sketchplugin/Contents/Sketch")[0] + ".sketchplugin/Contents/Resources/_webpack_resources/a19a7f950b94a9f2f51a5ed64ae0e156.html";
...
var sketch = __webpack_require__(/*! sketch/dom */ "sketch/dom"); // let Image = require('sketch/dom').Image;
// let async = require('sketch/async');
// let DataSupplier = require('sketch/data-supplier');
// let Settings = require('sketch/settings');


var Style = __webpack_require__(/*! sketch/dom */ "sketch/dom").Style; // let Document = require('sketch/dom').Document;
// let fiber = require('sketch/async').createFiber();


var webviewIdentifier = 'mockup.webview';
var mockupLayer = undefined;
var groupLayers = [];
var refreshLayer = undefined;
var selectedArtboard = undefined;
var selectedLayer = undefined;
/* harmony default export */ __webpack_exports__["default"] = (function (message) {
  // plugin window options
  var options = {
    resizable: false,
    closable: false,
    maximizable: false,
  ...
  function RepeatFunction() {
    ...
    if (refreshLayer !== selectedLayers.layers.length) {
      Object(sketch_module_web_view_remote__WEBPACK_IMPORTED_MODULE_1__["sendToWebview"])('ls_graphics', "refresh()");
      refreshLayer = selectedLayers.layers.length;
    }
  }
  ...
  function getImageBase64ForLayer(layer, size) {
    var symbolImg = sketch["export"](layer, {
      scales: size.toString(),
      format: 'png',
      output: false
    });
    return symbolImg.toString('base64');
  }

  function getPreview(layer) {
    var ArtboardImg = sketch["export"](layer, {
      scales: "0.3",
      format: 'png',
      output: false
    });
    return ArtboardImg.toString('base64');
  }
  ...
  function LoadArtboard() {
    ...
    groupLayers.splice(0, groupLayers.length);

    for (var i = 0; i < layers.length; i++) {
      if (layers[i].type === "Artboard") {
        groupLayers.push({
          id: layers[i].id,
          // src: `data:image/png;base64,${getPreview(layers[i])}`,
          title: layers[i].name
        });
      }
    } // if (symbols.length > 0) {
  ...
  browserWindow.webContents.on('get_element_src', function (id) {
    var document = sketch.getSelectedDocument();
    return "data:image/png;base64,".concat(getPreview(document.getLayerWithID(id)));
  });
  browserWindow.webContents.on('get-selected', function (s) {
    // var sl = context.actionContext.document.selectedLayers();
    // console.log(sl);
    var document = sketch.getSelectedDocument();
    var selectedLayers = document.selectedLayers;
  ...
  browserWindow.webContents.on('finish-distort', function (data) {
  ...
  //     sendToWebview('ls_graphics', `refresh()`);
  // });
  // browserWindow.on('blur', () => {
  //     sendToWebview('ls_graphics', `refresh()`);
  // });
  // browserWindow.on('focus', () => {
  //     sendToWebview('ls_graphics', `chekVector(${LoadVectors()})`);
  // });

  webContents.on('select_object', function () {
    return LoadArtboard();
  });
